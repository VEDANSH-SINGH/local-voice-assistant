...
export function useMeloTTS() {
  ...
  const getModelDirectoryForSource = useCallback(
    async (source: ModelSource) => {
      ...
      const dirName =
        source === "custom" ? "melo-tts-custom" : "melo-tts-default"
  ...
  const refreshDownloadedSources = useCallback(async () => {
    ...
    tLog(
      `Downloaded sources: default=${defaultDownloaded}, custom=${customDownloaded}`
    )
  ...
  const downloadAndExtractModels = useCallback(
    async (source: ModelSource = "default"): Promise<boolean> => {
      try {
        ...
        const modelUrl =
          source === "custom" ? MELO_TTS_CUSTOM_MODEL_URL : MELO_TTS_MODELS_URL
        ...
        if (!hasAnyModelAfter) {
          throw new Error(
            "Extraction verification failed - no model file found"
          )
        }
  ...
  const initializeModel = useCallback(
    async (modelId: string, sourceOverride?: ModelSource) => {
      ...
      try {
        ...
        tLog(
          `Initializing TTS model: ${model.label} from source: ${targetSource}`
        )
        ...
        if (!configFile.info().exists) {
          throw new Error(
            "tts_config.json not found in melo-tts-models directory"
          )
        }
